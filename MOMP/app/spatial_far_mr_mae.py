...
def spatial_far_mr_mae_map(dic=cfg, setting=setting):#, **kwargs):
    ...
    for combi in product(*layout_pool):
        ...
        metrics_df_dict, onset_da_dict = compute_metrics_multiple_years(**kwargs)
        
        # Create spatial metrics
        spatial_metrics = create_spatial_far_mr_mae(metrics_df_dict, onset_da_dict)
        
        if kwargs["save_nc_spatial_far_mr_mae"]:
            ...
            ds.attrs['model'] = f'case.model'
            ds.attrs['years'] = str(kwargs['years'])
            ds.attrs['tolerance_days'] = kwargs['tolerance_days']
            ds.attrs['verification_window'] = kwargs['verification_window']
            ds.attrs['forecast_days'] = kwargs['forecast_days']
            ds.attrs['max_forecast_day'] = kwargs['max_forecast_day']
            ds.attrs['mok_filter'] = int(bool((kwargs['mok'])))  # Convert boolean to integer (0 or 1)
    ...
    if plot_spatial_far_mr_mae:
        ...
        plot_sptial_metrics(spatial_metrics, **kwargs)
